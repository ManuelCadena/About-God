  ...
  const fetchLLMAnalysis = async () => {
    ...
    try {
      const analysis = await generatePanelAnalysis({
        panelId: 'watson-modelos',
        value: modelos.modelos?.filter(m => m.riesgo === 'critico' || m.riesgo === 'alto').length || 0,
        data: {
          seccion: modelos.seccion,
          gdd_actual: modelos.gdd_actual,
          fase_actual: modelos.fase_actual,
          plagas_criticas: modelos.modelos?.filter(m => m.riesgo === 'critico').map(m => m.plaga),
          plagas_alto: modelos.modelos?.filter(m => m.riesgo === 'alto').map(m => m.plaga),
          r2_promedio: modelos.modelos?.reduce((acc, m) => acc + m.r2_score, 0) / (modelos.modelos?.length || 1)
        }
      });
      ...
      <div className="mt-6 bg-gradient-to-br from-cyan-900/30 to-blue-900/30 rounded-xl p-6 border border-cyan-500/30">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-cyan-300">Interpretación Modelos Predictivos - Claude AI</p>
            </div>
          </div>
          <button
        ...
        <div className="mt-4 p-3 bg-gray-900/50 rounded-lg border border-gray-700">
          <p className="text-xs text-gray-400">
            <span className="text-white font-medium">Fuente:</span> Análisis generado por Claude AI basado en Watson Modelos Predictivos.
