...
import { Sprout, Zap, TrendingUp, Clock, RefreshCw, Scissors, Activity, Dna, Brain } from 'lucide-react'
  ...
  const fetchLLMAnalysis = async () => {
    ...
    try {
      const analysis = await generatePanelAnalysis({
        panelId: 'levin-morpho',
        value: data.morphogenesis_state.regeneration_potential,
        data: {
          perturbation_index: data.morphogenesis_state.perturbation_index,
          regeneration_potential: data.morphogenesis_state.regeneration_potential,
          meristem_activation_prob: data.morphogenesis_state.meristem_activation_prob,
          lai_recovery_rate: data.morphogenesis_state.lai_recovery_rate,
          field_state: data.morphogenesis_state.field_state,
          growth_direction: data.growth_direction,
          stress_growth_balance: data.stress_growth_balance,
          pruning_days_since: data.pruning_event.days_since,
          pruning_intensity: data.pruning_event.intensity,
          current_gdd: data.current_gdd,
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-300">Interpretación de Morfogénesis - Claude AI</p>
            </div>
          </div>
          <button
