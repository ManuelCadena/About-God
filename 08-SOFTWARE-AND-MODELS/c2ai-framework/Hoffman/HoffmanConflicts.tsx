  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Simulate conflict data (in production from API)
        const conflicts: Conflict[] = [
          {
            id: 'C001',
            agents: ['Health Agent', 'Nutrition Agent'],
            type: 'Resource',
            severity: 'high',
            description: 'Conflicto de timing: fertilización vs aplicación fitosanitaria',
            resolution: 'Priorizar Health Agent - IPF crítico (1.8) requiere acción inmediata. Retrasar fertilización 48h.',
            status: 'resolved',
            timestamp: new Date(Date.now() - 3600000).toISOString(),
            impactOnVEP: 0.05,
            impactOnPE: 0.02,
            resolvedBy: 'Meta-MDP Policy'
          },
          {
            id: 'C002',
            agents: ['Irrigation Agent', 'Harvest Agent'],
            type: 'Timing',
            severity: 'medium',
            description: 'Conflicto de ventana: riego pre-cosecha vs calidad de fruta',
            resolution: 'Ajustar riego a 15mm (50% de recomendación) para mantener calidad.',
            status: 'resolved',
            timestamp: new Date(Date.now() - 7200000).toISOString(),
            impactOnVEP: 0.02,
            impactOnPE: 0.01,
            resolvedBy: 'Weighted Average'
          },
          {
            id: 'C003',
            ...
            type: 'Strategy',
            severity: 'medium',
            description: 'Conflicto estratégico: poda vs desarrollo fenológico FEN-05',
            resolution: 'Pendiente análisis de impacto en floración próxima temporada.',
            status: 'pending',
          ...
          {
            id: 'C004',
            agents: ['Market Agent', 'Quality Agent'],
            type: 'Priority',
            severity: 'low',
            description: 'Conflicto de prioridad: precio vs calibre óptimo',
            resolution: 'Mantener calibre >70mm, precio secundario.',
            status: 'resolved',
            timestamp: new Date(Date.now() - 86400000).toISOString(),
            impactOnVEP: 0.01,
            impactOnPE: 0.01,
            resolvedBy: 'ROI Analysis'
          },
          {
            id: 'C005',
            agents: ['Davis S3', 'Davis S1'],
            type: 'Data',
            severity: 'low',
            description: 'Discrepancia de datos: temperatura S3 vs S1 (>2°C)',
            resolution: 'Usar promedio ponderado (S1: 70%, S3: 30%) por confianza de calibración.',
            status: 'resolved',
            timestamp: new Date(Date.now() - 43200000).toISOString(),
            impactOnVEP: 0.005,
            impactOnPE: 0.002,
            resolvedBy: 'Confidence Weighting'
          }
        ];
